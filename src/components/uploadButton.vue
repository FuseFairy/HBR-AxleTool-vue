  ...
  if (file && (file.type === 'image/jpeg' || file.type === 'image/jpg')) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        alert('The uploaded JPEG file format is incorrect or parameter parsing failed. Please check if the file is corrupted or if the parameter format is correct.');
      } finally {
        event.target.value = '';
      }
    };
