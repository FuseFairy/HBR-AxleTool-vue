...
export const useCharStore = defineStore('character_select', () => {
  const selections = ref(
    Object.fromEntries(Array.from({ length: 6 }, (_, i) => [i + 1, { character: null, style: null, skill: null, img: null }]))
  )

  const setSelection = (buttonKey, type, value) => {
    if (!selections.value[buttonKey]) {
      selections.value[buttonKey] = { character: null, style: null, skill: null, img: null }
    }
